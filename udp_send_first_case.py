...
from src.protocols import (
    INITIAL_CONDITION_CHECKSUM_SIZE,
...
def send_first_case_initial_condition() -> bytes:
    ...
    payload = client.send_initial_condition(first_case.initial_condition)
    ...
    print(f"expected_payload_size: {INITIAL_CONDITION_SIZE} bytes")
    checksum = int.from_bytes(
        payload[-INITIAL_CONDITION_CHECKSUM_SIZE:],
        byteorder="little" if INITIAL_CONDITION_ENDIAN == "<" else "big",
    )
    print(f"checksum_uint16: 0x{checksum:04X} ({checksum})")
...
if __name__ == "__main__":
    send_first_case_initial_condition()
